...
import { log, attachRequestId } from "./logger.js";
import { retry } from "./retry.js";
import {
  enabled as youtubeEnabled,
  getAuthUrl,
  exchangeCode,
  setTokens,
  updateVideoMetadata,
...
app.get("/budget/:videoId", (req: Request, res: Response) => {
  const { videoId } = req.params;
  res.json(budgetStatus(videoId));
});

app.post("/process", (req: Request & { id?: string }, res: Response) => {
...
app.get("/status/:videoId", (req: Request & { id?: string }, res: Response) => {
  const { videoId } = req.params;
...
app.get("/status/:videoId/stream", (req: Request, res: Response) => {
  const { videoId } = req.params;
...
app.get("/frames/:videoId", (req: Request, res: Response) => {
  const { videoId } = req.params;
  const video = store.videos.get(videoId);
  if (!video) return res.status(404).json({ error: "video not found" });
...
app.get("/videos/:videoId/summary", (req: Request, res: Response) => {
  const { videoId } = req.params;
  const video = store.videos.get(videoId);
  if (!video) return res.status(404).json({ error: "video not found" });
...
app.post("/frames/:videoId/extract", (req: Request, res: Response) => {
  const { videoId } = req.params;
...
app.post("/transcribe/:videoId", async (req: Request, res: Response) => {
  const { videoId } = req.params;
...
app.get("/transcript/:videoId", (req: Request, res: Response) => {
  const { videoId } = req.params;
  const rec = store.transcripts.get(videoId);
  if (!rec) return res.status(404).json({ error: "transcript not found" });
  res.json(rec);
});
app.post("/translate/:videoId", async (req: Request, res: Response) => {
  const { videoId } = req.params;
  ...
  if (!cap || (cap as any).videoId !== videoId)
...
app.post("/hooks/pubsub", (req: Request, res: Response) => {
  const body = req.body || {};
...
app.get("/recommendations/:videoId", (req: Request, res: Response) => {
  const { videoId } = req.params;
  const video = store.videos.get(videoId);
  if (!video) return res.status(404).json({ error: "video not found" });
...
app.post("/edit", (req: Request & { id?: string }, res: Response) => {
...
app.post("/publish", (req: Request & { id?: string }, res: Response) => {
...
function scoreCaption(text: string, keywords?: string[]) {
  const w = words(text);
...
function scoreThumbnail(style: string, title: string, keywords: string[]) {
  const base = 0.5;
      ...
      list.map(async (t) => {
        const ctr = vertexEnabled
          ? await predictCTR({
              ...
              colors:
                (t.visionFeatures && t.visionFeatures.dominantColors) || [],
            })
          : scoreThumbnail(
              t.style || "custom",
...
app.post("/captions/:videoId/score", (req: Request, res: Response) => {
  const { videoId } = req.params;
...
app.get("/thumbnails/:videoId", (req: Request, res: Response) => {
  const { videoId } = req.params;
  const video = store.videos.get(videoId);
  if (!video) return res.status(404).json({ error: "video not found" });
  const list = Array.from(store.thumbnails.values()).filter(
...
app.get("/captions/:videoId", (req: Request, res: Response) => {
  const { videoId } = req.params;
  const video = store.videos.get(videoId);
  if (!video) return res.status(404).json({ error: "video not found" });
  const list = Array.from(store.captions.values()).filter(
...
function isUnsafeVideo(videoId: string) {
  ...
  const styleUnsafe = thumbs.some((t) =>
    badWords.includes(String((t as any).style || "").toLowerCase())
  );
  const safeSearchUnsafe = thumbs.some((t) => {
    const ss =
      (t as any).visionFeatures && (t as any).visionFeatures.safeSearch;
    if (!ss) return false;
...
app.post("/safety/:videoId/check", (req: Request, res: Response) => {
  const { videoId } = req.params;
  const unsafe = isUnsafeVideo(videoId);
  res.json({ unsafe });
});

app.post("/youtube/publish", async (req: Request, res: Response) => {
...
app.post("/youtube/thumbnail", async (req: Request, res: Response) => {
...
app.post("/vision/:videoId/analyze", async (req: Request, res: Response) => {
  const { videoId } = req.params;
  const video = store.videos.get(videoId);
  if (!video) return res.status(404).json({ error: "video not found" });
  const list = Array.from(store.thumbnails.values()).filter(
...
app.post("/thumbnails/:videoId/generate", (req: Request, res: Response) => {
  const { videoId } = req.params;
  const { sourceFrameUri } = req.body || {};
  const video = store.videos.get(videoId);
  if (!video) return res.status(404).json({ error: "video not found" });
...
function updateRun(runId: string, updates: any) {
  const old = store.workflowRuns.get(runId);
  if (!old) return;
  const run = { ...old, ...updates };
  store.workflowRuns.set(runId, run);
}
function startPipeline(videoId: string) {
  const now = Date.now();
  ...
  if (pending)
    updateRun((pending as any).runId, {
      status: "completed",
      completedAt: now + 50,
    });

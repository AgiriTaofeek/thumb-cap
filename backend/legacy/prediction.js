...
const fetchRef = typeof fetch === 'function' ? fetch : null;

function heuristicCTR({ style, titleTokens = [], faces = 0, colors = [] }) {
  const base = 0.5;
  ...
  const faceBoost = Math.min(0.15, faces * 0.05);
  const score = Math.min(1, base + styleBoost + kwBoost + faceBoost);
  return Math.round(score * 100);
}

async function predictCTR(features) {
...
module.exports = { enabled, predictCTR };

...
import { enabled as youtubeEnabled, getAuthUrl, exchangeCode, setTokens, updateVideoMetadata, uploadThumbnail, } from "./youtube.js";
import { analyzeUri } from "./vision.js";
import { check as budgetCheck, commit as budgetCommit, getStatus as budgetStatus, } from "./cost.js";
...
app.post("/process", (req, res) => {
    ...
    if (pubsubEnabled)
        retry(() => publishMessage({
            type: "video_uploaded",
            videoId: id,
            gcsUri,
            title,
            language: language || "en",
        }), { retries: 2, baseMs: 300 }).catch(() => { });
    res.json({ videoId: id });
...
app.get("/frames/:videoId", (req, res) => {
    ...
    if (!video)
        return res.status(404).json({ error: "video not found" });
    const list = Array.from(store.frames.values()).filter((f) => f.videoId === videoId);
    res.json({ frames: list });
});
app.get("/videos", (req, res) => {
    const list = Array.from(store.videos.values()).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
...
app.post("/hooks/pubsub", (req, res) => {
    ...
    const now = Date.now();
    if (videoId)
        addRun(videoId, "pubsub_received", "completed", now, now);
...
app.post("/publish", (req, res) => {
    ...
    if (unsafe)
        return res.status(400).json({ error: "content flagged unsafe" });
...
function scoreCaption(text, keywords) {
    ...
    const engagement = Math.min(1, w.filter((x) => [
        "best",
        "pro",
        "ultimate",
        "free",
...
function scoreThumbnail(style, title, keywords) {
    ...
    const styleBoost = style === "preset-1"
        ? 0.1
        : style === "preset-2"
            ? 0.12
            : style === "preset-3"
...
app.post("/thumbnails/:videoId/score", async (req, res) => {
    const { videoId } = req.params;
    ...
    const scored = await Promise.all(list.map(async (t) => {
        const ctr = vertexEnabled
            ...
            : scoreThumbnail(t.style || "custom", title || video.title, keywords || []);
...
function isUnsafeVideo(videoId) {
    ...
    const capUnsafe = caps.some((c) => badWords.some((w) => String(c.text || "")
        .toLowerCase()
        .includes(w)));
    const styleUnsafe = thumbs.some((t) => badWords.includes(String(t.style || "").toLowerCase()));
...
app.post("/youtube/publish", async (req, res) => {
    ...
    if (!tokens)
        return res.status(401).json({ error: "no tokens for user" });
    setTokens(tokens);
    try {
...
function startPipeline(videoId) {
    const now = Date.now();
    const pending = Array.from(store.workflowRuns.values()).find((r) => r.videoId === videoId &&
    ...
    if (pending)
        ...
        addRun(videoId, "extracting_frames", "completed", now, now + 50);
    setTimeout(() => {
        const fNow = Date.now();
        const count = 5;
        for (let i = 0; i < count; i++) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                const t2b = Date.now();
                const listForVision = Array.from(store.thumbnails.values()).filter((x) => x.videoId === videoId);
                Promise.all(listForVision.map(async (t) => {
                    const vf = await analyzeUri(t.gcsUri);
                    const updated = { ...t, visionFeatures: vf };
                    store.thumbnails.set(t.variantId, updated);
                })).then(() => {
                    ...
                    const list = Array.from(store.thumbnails.values()).filter((x) => x.videoId === videoId);
                    const scored = list
                        .map((t) => {
